...
import pandas as pd
from main_train_model import test

model = tf.keras.models.load_model('model_transfer_crop.h5')
st.title("Face mask detection")

model_2 = load("svc_clf.joblib")

menu = ["Machine learning", "Neural network"]
	...
	if image_file is not None:
		...
		if face_detection_results.detections:
			for face_no, face in enumerate(face_detection_results.detections):
				...
				data = {"name":"img.jpg","x1":[xleft], "x2":[xtop], "y1":[xright], "y2":[xbottom]}

				image_pd = pd.DataFrame(data)
				
				
				prediction_2 = test(image_pd, im)
				if prediction_2 == "face_no_mask":
					st.write("No mask")
				else:
					st.write("Mask")
